...
#PowerShell 7 is just PowerShell
#
...
get-services | select name, status | out-file c:\data\services.txt # writes to file just like seen on the screen
...
get-server | where {$_.name -eq "w32time"}
...
get-service | where status -eq "Running" #possible with powershell 3 or newer
...
#quick talk about pipeline was errors are not sent down the pipeline only successful objects
